...
# The sample contacts older builds seeded into every new account. Matched on
# (email, name) so a real contact who merely shares one of these addresses
# survives the cleanup below.
_LEGACY_DEMO_CONTACTS = [
    ("sarah.chen@gmail.com", "Sarah Chen"),
    ("mike@acmecorp.com", "Mike Doyle"),
    ("info@example.com", "Info Desk"),
    ("jdoe@yahoo.com", "Jane Doe"),
    ("bounced@nodomain.invalid", "Old Address"),
    ("unsub@gmail.com", "Tom Reed"),
]


def _remove_demo_contacts():
    """One-time cleanup: drop the demo contacts previously seeded into every
    account (contacts are no longer seeded at all), plus the now-empty
    'Newsletter Subscribers' starter list."""
    db = get_db()
    for email, name in _LEGACY_DEMO_CONTACTS:
        db.execute("DELETE FROM contacts WHERE email=? AND name=?", (email, name))
    db.execute(
        "DELETE FROM contact_lists WHERE name='Newsletter Subscribers' AND NOT EXISTS"
        " (SELECT 1 FROM contacts c WHERE c.list_id=contact_lists.id)")
    db.commit()
...
def seed_demo(account_id, user_email):
    ...
    Contacts are deliberately NOT seeded — accounts start with an empty
    contact book and only ever hold contacts the user added themselves."""
    n = now()
    # campaigns
    campaigns = [
