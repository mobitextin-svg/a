...
def execute_campaign_send(account_id, campaign_id, base_url):
    ...
    for i, contact in enumerate(recipients):
        ...
        subj = SEND.render_subject(camp["subject"], dict(contact))
        cfg = SEND.server_transport(rows_by_id[sid]) or env_cfg
        status, err = "dry-run", None
        if cfg:
            ok, info = SEND.smtp_send(cfg, contact["email"], subj, html,
...
def create_app():
    ...
    register_api(app)
    # v73: guided onboarding wizard (additive module — see onboarding_v73.py)
    try:
        from .onboarding_v73 import (register_onboarding, register_auth_extras,
                                     register_experience,
                                     register_oauth, register_activity_feed)
    except ImportError:
        from onboarding_v73 import (register_onboarding, register_auth_extras,
                                    register_experience,
                                    register_oauth, register_activity_feed)
    register_onboarding(app, current_user, current_account)
    register_auth_extras(app, current_user, current_account)
    register_experience(app, current_user, current_account)
    register_oauth(app, current_user, current_account)
    register_activity_feed(app, current_user, current_account)
    # v82: email OTP + mobile OTP verification (additive — see auth_v81.py)
    try:
        from .auth_v81 import register_auth_v81
    except ImportError:
        from auth_v81 import register_auth_v81
    register_auth_v81(app, current_user, current_account, _deliver_email,
...
_COMMON_WEAK_PASSWORDS = {"password", "password1", "123456", "12345678",
    "123456789", "qwerty", "qwerty123", "111111", "123123", "abc123",
    "letmein", "welcome", "admin123", "iloveyou", "monkey", "dragon",
    "football", "passw0rd", "changeme", "000000", "1234567890"}


def _password_ok(pw, email=None):
    if len(pw) < 8:
    ...
    if classes < 3:
        return False, "Password must mix at least 3 of: uppercase, lowercase, numbers, symbols."
    if email and pw.lower() in email.lower():
        return False, "Password shouldn't contain your email address."
    return True, ""
...
def register_auth(app):
    ...
    def signup():
        ...
        if request.method == "POST":
            # Throttle signup abuse (bot account creation) per IP.
            if not app.config.get("TESTING") and not rate_limit(
                    f"signup:{request.remote_addr}", limit=8, window=600):
                flash("Too many signup attempts from this network. Please try "
                      "again later.", "error")
            ...
            if not (name and email and "@" in email):
                flash("Please provide a name and a valid email address.", "error")
                return render_template("signup.html")
            ok, msg = _password_ok(pw, email)
            if not ok:
                flash(msg, "error")
                return render_template("signup.html")
            if D.query("SELECT 1 FROM users WHERE email=?", (email,), one=True):
    ...
    def verify_email(token):
        ...
        cu = current_user()
        if cu and cu["id"] == u["id"]:
            # v82: continue the guided flow — offer the mobile-number step next.
            try:
                fresh = D.query("SELECT * FROM users WHERE id=?", (u["id"],), one=True)
                if not fresh["mobile_verified"]:
                    return redirect(url_for("verify_mobile_page"))
            except Exception:  # noqa: BLE001  (column absent on odd builds)
                pass
        return redirect(url_for("dashboard") if cu else url_for("login"))
    ...
    def reset(token):
        ...
        if request.method == "POST":
            ...
            ok, msg = _password_ok(new, u["email"])
            if not ok:
                flash(msg, "error")
...
def register_modules(app):
    # ---- Dashboard (removed) --------------------------------------------- #
    # The endpoint stays (login/signup/2FA flows land on it) but the page is
    # gone — everything now starts from the consolidated Contacts screen.
    @app.route("/dashboard")
    @login_required
    def dashboard():
    ...
    # Tag colors are assigned automatically (no manual picker). The palette is
    # tried in order; each new tag takes the first colour not already in use so
    # tags stay visually distinct. Once every colour is used, the least-used one
    # is reused.
    TAG_COLORS = ["blue", "red", "green", "yellow", "purple", "gray"]

    def _auto_tag_color(existing_colors):
        from collections import Counter
        counts = Counter(c for c in existing_colors if c)
        for c in TAG_COLORS:
            if counts.get(c, 0) == 0:
                return c
        return min(TAG_COLORS, key=lambda c: counts.get(c, 0))

    def _pick_tag_color(aid):
        """Next unique colour for a brand-new tag on this account."""
        rows = D.query("SELECT color FROM account_tags WHERE account_id=?", (aid,))
        return _auto_tag_color([r["color"] for r in rows])
    ...
    def contacts():
        ...
        if request.method == "POST":
            ...
            # ---------- Import (de-duplicated, with summary) ---------- #
                ...
                imp_tag = request.form.get("import_tag", "").strip()
                for rec in records:
            ...
            # ---------- Contact-level actions ---------- #
                ...
                for cid in ids:
                    ...
                    if add_t and add_t not in tags:
                        tags.append(add_t)
                        D.log_contact_event(aid, cid, "tag_added", add_t)
                    if del_t and del_t in tags:
                        tags.remove(del_t)
                        D.log_contact_event(aid, cid, "tag_removed", del_t)
                    D.execute("UPDATE contacts SET tags=? WHERE id=? AND account_id=?",
                              (", ".join(tags), cid, aid))
                if add_t:
                    D.touch_recent_tag(aid, add_t)
                if ids:
                    flash(f"Tags updated on {len(ids)} contact(s).", "success")
            elif action == "create_tag":
                new_tag = (request.form.get("tag_name") or "").strip()[:60]
                desc = (request.form.get("tag_description") or "").strip()[:200]
                if new_tag:
                    color = _pick_tag_color(aid)   # auto, unique
                    try:
                        D.execute("INSERT INTO account_tags (account_id, name, color,"
                                  " description, created_at) VALUES (?,?,?,?,?)",
                                  (aid, new_tag, color, desc, D.now()))
                        flash(f"Tag '{new_tag}' created.", "success")
                    except Exception:
                        flash(f"Tag '{new_tag}' already exists.", "info")
                else:
                    flash("Enter a tag name.", "error")
            elif action == "update_tag_meta":
                # Update a tag's color / description from the Tag Detail panel.
                t_name = (request.form.get("tag_name") or "").strip()
                color = (request.form.get("tag_color") or "blue").strip()
                desc = (request.form.get("tag_description") or "").strip()[:200]
                if t_name:
                    D.execute("UPDATE account_tags SET color=?, description=?"
                              " WHERE account_id=? AND name=?", (color, desc, aid, t_name))
                    flash(f"Tag '{t_name}' updated.", "success")
            elif action == "toggle_favorite_tag":
                t_name = (request.form.get("tag_name") or "").strip()
                if t_name:
                    row = D.query("SELECT favorite FROM account_tags WHERE account_id=? AND name=?",
                                  (aid, t_name), one=True)
                    new_fav = 0 if row and row["favorite"] else 1
                    D.execute("UPDATE account_tags SET favorite=? WHERE account_id=? AND name=?",
                              (new_fav, aid, t_name))
            elif action == "rename_tag":
                old_tag = (request.form.get("old_tag") or "").strip()
                new_tag = (request.form.get("new_tag") or "").strip()[:60]
                new_desc = (request.form.get("tag_description") or "").strip()[:200]
                if old_tag and new_tag:
                    updated = 0
                    if old_tag != new_tag:
                        rows = D.query("SELECT id, tags FROM contacts WHERE account_id=?"
                                       " AND tags LIKE ?", (aid, f"%{old_tag}%"))
                        for row in rows:
                            tags = [t.strip() for t in (row["tags"] or "").split(",") if t.strip()]
                            if old_tag in tags:
                                ...
                                D.log_contact_event(aid, row["id"], "tag_removed", old_tag)
                                D.log_contact_event(aid, row["id"], "tag_added", new_tag)
                                updated += 1
                    old_meta = D.query("SELECT favorite, color FROM account_tags"
                                       " WHERE account_id=? AND name=?", (aid, old_tag), one=True)
                    # Colour is preserved from the old tag (assigned automatically when
                    # the tag was first created); never chosen by hand on rename.
                    new_color = (old_meta["color"] if old_meta and old_meta["color"]
                                 else _pick_tag_color(aid))
                    D.execute("DELETE FROM account_tags WHERE account_id=? AND name=?", (aid, old_tag))
                    try:
                        D.execute("INSERT INTO account_tags (account_id, name, color,"
                                  " description, favorite, created_at) VALUES (?,?,?,?,?,?)",
                                  (aid, new_tag, new_color, new_desc,
                                   old_meta["favorite"] if old_meta else 0, D.now()))
                    except Exception:
                        D.execute("UPDATE account_tags SET color=?, description=?"
                                  " WHERE account_id=? AND name=?",
                                  (new_color, new_desc, aid, new_tag))
                    if old_tag != new_tag:
                        flash(f"Tag '{old_tag}' renamed to '{new_tag}' on {updated} contact(s).", "success")
                    else:
                        flash(f"Tag '{new_tag}' updated.", "success")
                else:
                    flash("Enter valid old and new tag names.", "error")
            elif action == "delete_tag":
                del_tag = (request.form.get("tag_name") or "").strip()
                mode = request.form.get("delete_mode", "tag_only")  # tag_only | tag_and_contacts
                if del_tag:
                    if mode == "tag_and_contacts":
                        # Delete contacts that have ONLY this tag (no other tags)
                        rows = D.query("SELECT id, tags FROM contacts WHERE account_id=?"
                                       " AND tags LIKE ?", (aid, f"%{del_tag}%"))
                        delete_ids = []
                        keep_rows = []
                        for row in rows:
                            tags = [t.strip() for t in (row["tags"] or "").split(",") if t.strip()]
                            if del_tag in tags:
                                other = [t for t in tags if t != del_tag]
                                if not other:
                                    delete_ids.append(row["id"])
                                else:
                                    keep_rows.append((row["id"], other))
                        if delete_ids:
                            qs = ",".join("?" for _ in delete_ids)
                            D.execute(f"DELETE FROM contacts WHERE account_id=? AND id IN ({qs})",
                                      [aid] + delete_ids)
                        for rid, remaining in keep_rows:
                            D.execute("UPDATE contacts SET tags=? WHERE id=? AND account_id=?",
                                      (", ".join(remaining), rid, aid))
                            D.log_contact_event(aid, rid, "tag_removed", del_tag)
                        D.execute("DELETE FROM account_tags WHERE account_id=? AND name=?", (aid, del_tag))
                        flash(f"Tag '{del_tag}' deleted. {len(delete_ids)} contact(s) removed.", "success")
                    else:
                        # Remove tag from all contacts, keep the contacts
                        rows = D.query("SELECT id, tags FROM contacts WHERE account_id=?"
                                       " AND tags LIKE ?", (aid, f"%{del_tag}%"))
                        updated = 0
                        for row in rows:
                            tags = [t.strip() for t in (row["tags"] or "").split(",") if t.strip()]
                            if del_tag in tags:
                                tags = [t for t in tags if t != del_tag]
                                D.execute("UPDATE contacts SET tags=? WHERE id=? AND account_id=?",
                                          (", ".join(tags), row["id"], aid))
                                D.log_contact_event(aid, row["id"], "tag_removed", del_tag)
                                updated += 1
                        D.execute("DELETE FROM account_tags WHERE account_id=? AND name=?", (aid, del_tag))
                        flash(f"Tag '{del_tag}' deleted. {updated} contact(s) untagged.", "success")
                else:
                    flash("No tag specified.", "error")
            elif action == "move_tag_contacts":
                from_tag = (request.form.get("from_tag") or "").strip()
                to_tag = (request.form.get("to_tag") or "").strip()[:60]
                if from_tag and to_tag and from_tag != to_tag:
                    rows = D.query("SELECT id, tags FROM contacts WHERE account_id=?"
                                   " AND tags LIKE ?", (aid, f"%{from_tag}%"))
                    updated = 0
                    for row in rows:
                        tags = [t.strip() for t in (row["tags"] or "").split(",") if t.strip()]
                        if from_tag in tags:
                            ...
                            D.execute("UPDATE contacts SET tags=? WHERE id=? AND account_id=?",
                                      (", ".join(tags), row["id"], aid))
                            D.log_contact_event(aid, row["id"], "tag_removed", from_tag)
                            D.log_contact_event(aid, row["id"], "tag_added", to_tag)
                            updated += 1
                    D.touch_recent_tag(aid, to_tag)
                    flash(f"Moved {updated} contact(s) from '{from_tag}' to '{to_tag}'.", "success")
                else:
                    flash("Pick two different tags.", "error")
            elif action == "merge_tags":
                tag_a = (request.form.get("tag_a") or "").strip()
                tag_b = (request.form.get("tag_b") or "").strip()
                merged = (request.form.get("merged_tag") or "").strip()[:60]
                if tag_a and tag_b and merged:
                    rows = D.query("SELECT id, tags FROM contacts WHERE account_id=?"
                    ...
                    for row in rows:
                        ...
                        changed = False
                        if tag_a in tags:
                            tags = [t for t in tags if t != tag_a]
                            changed = True
                        if tag_b in tags:
                            tags = [t for t in tags if t != tag_b]
                            changed = True
                        if changed:
                            ...
                            D.execute("UPDATE contacts SET tags=? WHERE id=? AND account_id=?",
                                      (", ".join(tags), row["id"], aid))
                            D.log_contact_event(aid, row["id"], "tag_removed", f"{tag_a}, {tag_b}")
                            D.log_contact_event(aid, row["id"], "tag_added", merged)
                            updated += 1
                    D.execute("DELETE FROM account_tags WHERE account_id=? AND name IN (?,?)",
                              (aid, tag_a, tag_b))
                    try:
                        D.execute("INSERT INTO account_tags (account_id, name, created_at)"
                                  " VALUES (?,?,?)", (aid, merged, D.now()))
                    except Exception:
                        pass
                    D.touch_recent_tag(aid, merged)
        ...
        if sel not in ("suppression", "trash"):
            ...
            if tag_filter:
                # Filter contacts that have this exact tag in their comma-separated tags field.
        ...
        # Collect distinct tags with full metadata, merging registry + usage.
        reg_tags = D.query("SELECT name, color, description, favorite, last_used_at,"
                           " created_at FROM account_tags WHERE account_id=?", (aid,))
        reg_by_name = {r["name"]: dict(r) for r in reg_tags}
        tag_counts_dict = {r["name"]: 0 for r in reg_tags}
        tag_rows = D.query("SELECT tags FROM contacts WHERE account_id=? AND tags IS NOT NULL AND tags != ''", (aid,))
        for _tr in tag_rows:
            for _t in (_tr["tags"] or "").split(","):
                _t = _t.strip()
                if _t:
                    tag_counts_dict[_t] = tag_counts_dict.get(_t, 0) + 1
                    # Auto-register tags found in contacts but not yet in registry
                    if _t not in reg_by_name:
                        try:
                            _color = _auto_tag_color([m.get("color")
                                                      for m in reg_by_name.values()])
                            D.execute("INSERT INTO account_tags (account_id, name, color,"
                                      " created_at) VALUES (?,?,?,?)",
                                      (aid, _t, _color, D.now()))
                            reg_by_name[_t] = {"name": _t, "color": _color, "description": "",
                                               "favorite": 0, "last_used_at": None, "created_at": D.now()}
                        except Exception:
                            pass
        # all_tags: list of dicts sorted favorites-first, then alphabetically.
        all_tags_full = []
        for name, count in tag_counts_dict.items():
            meta = reg_by_name.get(name, {})
            all_tags_full.append({
        ...
        all_tags_full.sort(key=lambda t: (not t["favorite"], t["name"].lower()))
        # Back-compat tuple list (name, count) still used by a couple of older spots.
        all_tags = [(t["name"], t["count"]) for t in all_tags_full]
        recent_tags = [t for t in (acct["recent_tags"] or "").split(",") if t][:8]
        return render_template(
            ...
            type_icons=LIST_TYPE_ICONS,
            all_tags=all_tags,
            all_tags_full=all_tags_full,
            recent_tags=recent_tags,
            tag_filter=tag_filter,
            list_tags={l["id"]: [t.strip() for c in D.query(
                "SELECT tags FROM contacts WHERE account_id=? AND list_id=? AND tags IS NOT NULL AND tags != ''",
                (aid, l["id"])) for t in (c["tags"] or "").split(",") if t.strip()]
                for l in lists},
    ...
    @app.route("/contacts/<int:cid>/timeline")
    @login_required
    def contact_timeline(cid):
        """Full chronological timeline for one contact: imports, tag changes,
        notes, campaign sends/opens/clicks/bounces — merged & sorted by time."""
        aid = current_account()["id"]
        c = D.query("SELECT * FROM contacts WHERE id=? AND account_id=?",
                    (cid, aid), one=True)
        if not c:
        ...
        # Contact-level events (imported / tag_added / tag_removed / note)
        for e in D.query("SELECT kind, detail, created_at FROM contact_events"
                         " WHERE account_id=? AND contact_id=? ORDER BY id",
                         (aid, cid)):
            label = {
                "imported": f"Imported (source: {e['detail'] or 'manual'})",
                "tag_added": f"Tag added: {e['detail']}",
                "tag_removed": f"Tag removed: {e['detail']}",
                "note": "Note added",
            }.get(e["kind"], e["kind"])
            events.append({"type": e["kind"], "label": label,
                           "detail": e["detail"], "at": e["created_at"]})

        # Campaign-level events derived from messages (sent/opened/clicked/bounced)
        msgs = D.query("SELECT m.campaign_id, m.status, m.opened, m.clicked,"
                       " m.opened_at, m.clicked_at, m.created_at, c.name AS cname"
                       " FROM messages m LEFT JOIN campaigns c ON c.id=m.campaign_id"
                       " WHERE m.account_id=? AND m.email=? ORDER BY m.id",
                       (aid, email))
        for m in msgs:
            cname = m["cname"] or "Campaign"
            events.append({"type": "campaign_sent",
                          "label": f"Campaign sent: {cname}",
                          "detail": cname, "at": m["created_at"]})
            if m["opened"]:
                events.append({"type": "campaign_opened",
                              "label": f"Opened: {cname}",
                              "detail": cname, "at": m["opened_at"] or m["created_at"]})
            if m["clicked"]:
                events.append({"type": "campaign_clicked",
                              "label": f"Clicked: {cname}",
                              "detail": cname, "at": m["clicked_at"] or m["created_at"]})
            if m["status"] == "failed":
                events.append({"type": "campaign_bounced",
                              "label": f"Bounced: {cname}",
                              "detail": cname, "at": m["created_at"]})

        # Replies (complaints table also tracks unsubscribe/complaint actions)
        for r in D.query("SELECT kind, created_at FROM complaints WHERE"
                         " account_id=? AND email=? ORDER BY id", (aid, email)):
            label = {"complaint": "Marked as spam",
                     "unsubscribe": "Unsubscribed"}.get(r["kind"], r["kind"])
            events.append({"type": r["kind"], "label": label,
                          "detail": "", "at": r["created_at"]})

        # Current note (single field, shown as the latest note state)
        if c["note"]:
            events.append({"type": "note", "label": "Note",
                          "detail": c["note"], "at": c["created_at"]})
    ...
    def tag_detail(tag_name):
        """Full Tag Detail stats: total contacts, color, description, dates,
        campaign count, open/click rate for contacts carrying this tag."""
        aid = current_account()["id"]
        meta = D.query("SELECT * FROM account_tags WHERE account_id=? AND name=?",
                       (aid, tag_name), one=True)
        contacts = D.query("SELECT id, email FROM contacts WHERE account_id=?"
                           " AND tags LIKE ?", (aid, f"%{tag_name}%"))
        # Exact match (LIKE is a coarse pre-filter since tags are comma-joined text)
        tagged = [c for c in contacts]  # tags LIKE already narrows; exact-match below
        tagged_emails = set()
        exact_ids = []
        for c in contacts:
            full = D.query("SELECT tags FROM contacts WHERE id=?", (c["id"],), one=True)
            tlist = [t.strip() for t in (full["tags"] or "").split(",") if t.strip()]
            if tag_name in tlist:
                tagged_emails.add(c["email"])
                exact_ids.append(c["id"])
        total_contacts = len(exact_ids)
        campaign_ids = set()
        opens = clicks = sent = 0
        if tagged_emails:
            qs = ",".join("?" for _ in tagged_emails)
            msgs = D.query(f"SELECT campaign_id, opened, clicked FROM messages"
                           f" WHERE account_id=? AND email IN ({qs})",
                           [aid] + list(tagged_emails))
            sent = len(msgs)
            opens = sum(1 for m in msgs if m["opened"])
            clicks = sum(1 for m in msgs if m["clicked"])
            campaign_ids = {m["campaign_id"] for m in msgs}
        return jsonify({
            "name": tag_name,
            "color": meta["color"] if meta else "blue",
            ...
            "open_rate": round(100 * opens / sent, 1) if sent else 0,
            "click_rate": round(100 * clicks / sent, 1) if sent else 0,
        })

    @app.route("/contact-lists")
    @login_required
    def contact_lists():
        """Merged into the consolidated Contacts page (lists rail)."""
        return redirect(url_for("contacts"))

    @app.route("/tags")
    @login_required
    def tags_page():
        """Merged into the consolidated Contacts page (tags rail)."""
        return redirect(url_for("contacts"))
    ...
    def campaigns():
        ...
        if request.method == "POST":
            action = request.form.get("action")
            if action in ("create", "create_send"):
                ...
                if action == "create_send":
                    # Save & send now — create the draft, then fire it immediately.
    ...
    def _resolve_recipients(aid, mode, campaign_name):
        ...
        if mode == "listtag":
            # List-first picker: choose whole lists and/or specific tags WITHIN a
            # list. Sends to the union of everyone matched, deduplicated by email.
            #   list_ids    -> every active contact in that list (whole list)
            #   scoped_tags -> "lid::tag" — active contacts in list `lid` tagged `tag`
            #   rcpt_tags   -> account-wide tag (kept for backward compatibility)
            lids = [l for l in request.form.getlist("list_ids") if l]
            tags = [t for t in request.form.getlist("rcpt_tags") if t]
            scoped = []
            for s in request.form.getlist("scoped_tags"):
                if s and "::" in s:
                    _lid, _tag = s.split("::", 1)
                    if _lid.strip() and _tag.strip():
                        scoped.append((_lid.strip(), _tag.strip()))
            if not lids and not tags and not scoped:
                return (None, 0, "Select at least one list or tag.")
            for lid in lids:
                if not D.query("SELECT 1 FROM contact_lists WHERE id=? AND account_id=?",
                               (lid, aid), one=True):
                    return (None, 0, "Invalid contact list selection.")
            for _lid, _ in scoped:
                if not D.query("SELECT 1 FROM contact_lists WHERE id=? AND account_id=?",
                               (_lid, aid), one=True):
                    return (None, 0, "Invalid contact list selection.")
            # Fast path: exactly one whole list, nothing else — reuse it directly.
            if len(lids) == 1 and not tags and not scoped:
                lid = int(lids[0])
                n = D.query("SELECT COUNT(*) c FROM contacts WHERE account_id=? AND"
                            " list_id=? AND status='active'", (aid, lid), one=True)["c"]
                return (lid, n, None)
            all_active = D.query("SELECT id, email, name, tags, list_id FROM contacts"
                                 " WHERE account_id=? AND status='active'", (aid,))
            lid_set = {int(l) for l in lids}
            scoped_set = {(int(l), t) for l, t in scoped}
            matched, seen_emails = [], set()
            for c in all_active:
                ctags = [t.strip() for t in (c["tags"] or "").split(",") if t.strip()]
                in_list = c["list_id"] in lid_set
                in_tag = any(t in ctags for t in tags)
                in_scoped = any(c["list_id"] == sl and st in ctags
                                for sl, st in scoped_set)
                if (in_list or in_tag or in_scoped) and c["email"] not in seen_emails:
                    matched.append(c)
                    seen_emails.add(c["email"])
            if not matched:
                return (None, 0, "No active contacts found for the selected list(s)/tag(s).")
            label_bits = []
            if lids:
                label_bits.append(f"{len(lids)} list(s)")
            if scoped:
                label_bits.append("+".join(sorted({t for _, t in scoped})))
            if tags:
                label_bits.append("+".join(tags))
            base = f"{campaign_name} ({', '.join(label_bits)})"[:55]
            mname, i = base, 2
            ...
            for c in matched:
                D.execute("INSERT INTO contacts (account_id, list_id, email, name, tags,"
                          " status, created_at) VALUES (?,?,?,?,?,?,?)",
                          (aid, mlid, c["email"], c["name"], c["tags"], "active", D.now()))
            return (mlid, len(matched), None)
        if mode == "all":
            n = D.query("SELECT COUNT(*) c FROM contacts WHERE account_id=? AND"
                        " status='active'", (aid,), one=True)["c"]
            return (None, n, None)
    ...
    def campaign_wizard():
        ...
        all_active = D.query("SELECT COUNT(*) c FROM contacts WHERE account_id=? AND"
                             " status='active'", (aid,), one=True)["c"]
        # Tags for the "By Tag" recipient card, with active-contact counts.
        _tag_rows = D.query("SELECT tags FROM contacts WHERE account_id=? AND"
                            " status='active' AND tags IS NOT NULL AND tags != ''", (aid,))
        _tag_counts = {}
        ...
        rcpt_tags = sorted(
            ({"name": n, "n": c} for n, c in _tag_counts.items()),
            key=lambda x: x["name"].lower())
        # Per-list tag map: which tags appear within each list's active contacts,
        # with the count of contacts in THAT list carrying each tag. Powers the
        # list-first recipient picker (expand a list → pick whole list or a tag).
        _list_tag_rows = D.query("SELECT list_id, tags FROM contacts WHERE account_id=?"
                                 " AND status='active' AND tags IS NOT NULL AND tags != ''",
                                 (aid,))
        _ltm_counts = {}
        for _r in _list_tag_rows:
            if _r["list_id"] is None:
                continue
            bucket = _ltm_counts.setdefault(str(_r["list_id"]), {})
            for _t in (_r["tags"] or "").split(","):
                _t = _t.strip()
                if _t:
                    bucket[_t] = bucket.get(_t, 0) + 1
        _tag_color_map = {r["name"]: (r["color"] or "blue") for r in D.query(
            "SELECT name, color FROM account_tags WHERE account_id=?", (aid,))}
        list_tag_map = {
            lid: [{"name": n, "n": c, "color": _tag_color_map.get(n, "blue")}
                  for n, c in sorted(d.items(), key=lambda x: x[0].lower())]
            for lid, d in _ltm_counts.items()
        }
    ...
    def _sample_contact(aid, to_email=None, contact_id=None):
        ...
        is given, use that exact contact (true single-contact testing with real
        ...
        if to_email:
            base["email"] = to_email
        # Fill any still-empty fields with friendly placeholders so nothing
        # renders blank in a preview.
        base["name"] = base["name"] or (base["email"].split("@")[0] if base["email"] else "there")
        base["company"] = base["company"] or "Your Company"
    ...
    def _first_transport(aid):
        """Return (cfg, source) for the first usable SMTP transport, or
        (None, None) when the account is in dry-run (no relay configured)."""
        for r in D.query("SELECT * FROM smtp_servers WHERE account_id=? ORDER BY id",
                         (aid,)):
            cfg = SEND.server_transport(r)
            if cfg:
                return cfg, r["name"]
        env = SEND.env_transport()
        if env:
            return env, "environment relay"
        return None, None
    ...
    def sender():
        aid = current_account()["id"]
        # Real, reusable templates for the "pick a campaign" dropdown — each
        # carries its saved subject + body so selecting one auto-fills compose.
        tpls = D.query("SELECT id, name, subject, content FROM templates"
                       " WHERE account_id=? AND kind='Email'"
                       " AND COALESCE(trashed,0)=0 ORDER BY COALESCE(favorite,0) DESC,"
                       " name", (aid,))
        ready = D.query("SELECT id, name, subject, content FROM system_templates"
                        " WHERE published=1 ORDER BY category, name")
        # Contact lists for the audience selector (null = all active contacts).
        lists = D.query("SELECT cl.id, cl.name,"
                        " (SELECT COUNT(*) FROM contacts c WHERE c.list_id=cl.id"
                        "  AND c.status='active') AS n"
                        " FROM contact_lists cl WHERE cl.account_id=? ORDER BY cl.name",
                        (aid,))
        total_active = D.query("SELECT COUNT(*) c FROM contacts WHERE account_id=?"
                               " AND status='active'", (aid,), one=True)["c"]
        # Legacy display names for the right-hand feature grid.
        templates = [t["name"] for t in tpls] or ["Welcome Email", "Newsletter",
                     "Promotion", "Re-engagement", "Transactional Receipt"]
        queue = D.query("SELECT * FROM campaigns WHERE account_id=? AND status IN"
                        " ('Running','Scheduled') ORDER BY id DESC", (aid,))
        # A short list of real contacts to test against (variables fill from the
        # chosen one). Capped so the dropdown stays light.
        contacts = D.query("SELECT id, name, email FROM contacts WHERE account_id=?"
                           " AND status='active' ORDER BY id LIMIT 200", (aid,))
        _u = current_user()
        return render_template("sender.html", templates=templates, queue=queue,
                               tpls=tpls, ready=ready, lists=lists,
                               total_active=total_active, contacts=contacts,
                               test_to=(_u["email"] if _u else ""))

    # Live preview: render the current subject + body against a sample contact
    # so the user sees exactly what a recipient gets — variables resolved,
    # links wrapped, footer appended. Never sends anything.
    @app.route("/sender/preview", methods=["POST"])
    @login_required
    def sender_preview():
        aid = current_account()["id"]
        subject = request.form.get("subject", "")
        body = request.form.get("body", "")
        sample = _sample_contact(aid, contact_id=request.form.get("contact_id") or None)
        base = request.host_url.rstrip("/")
        html = SEND.render_html(body, dict(sample), base, SEND.make_token())
        subj = SEND.render_subject(subject, dict(sample))
        return jsonify({"ok": True, "subject": subj, "html": html,
                        "using": {"name": sample["name"], "email": sample["email"]}})

    # Send exactly one test email to a chosen address. Does NOT create a
    # campaign, touch the queue, or move any counters — it is a throwaway
    # sample that proves real inbox rendering + relay health.
    @app.route("/sender/test", methods=["POST"])
    @login_required
    def sender_test():
        aid = current_account()["id"]
        ...
        if not re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", to):
            return jsonify({"ok": False, "msg": "Enter a valid test email address."})
        cfg, source = _first_transport(aid)
        if not cfg:
            return jsonify({"ok": False, "msg": "No SMTP relay configured — add one "
                            "under SMTP settings to send a real test. (You're in "
                            "dry-run mode, so nothing would actually arrive.)"})
        sample = _sample_contact(aid, to_email=to,
                                 contact_id=request.form.get("contact_id") or None)
        base = request.host_url.rstrip("/")
        html = SEND.render_html(body, dict(sample), base, SEND.make_token())
        subj = "[TEST] " + SEND.render_subject(subject, dict(sample))
        _u = current_user()
        _from = _u["email"] if _u else None
        try:
            ok, info = SEND.smtp_send(cfg, to, subj, html, from_addr=_from)
        except Exception as exc:  # noqa: BLE001
            return jsonify({"ok": False, "msg": f"Test send failed: {exc}"})
        if ok:
            D.log_activity(aid, _from or "system", f"Sent a test email to {to}")
            return jsonify({"ok": True, "msg": f"Test sent to {to} via {source}. "
                            "Check your inbox (and spam folder)."})
        return jsonify({"ok": False, "msg": f"Relay rejected the test: {info}"})
    ...
    def settings():
        ...
        if request.method == "POST":
            ...
            if action == "profile":
                ...
                pw_ok, pw_msg = _password_ok(new, u["email"])
    ...
    def admin():
        ...
        def _purge_account(acct_id):
            """Permanently remove a workspace and everything scoped to it, so a
            hard delete never leaves orphaned rows behind."""
            tables = ("verifications", "contact_lists", "contacts", "blocked_emails",
                      "contact_fields", "campaigns", "smtp_servers", "domains",
                      "api_keys", "activity", "invoices", "templates",
                      "template_folders", "automations", "landing_pages", "webhooks",
                      "login_history", "burst_jobs", "messages", "complaints",
                      "api_logs", "burst_purchases", "campaign_attachments",
                      "contact_imports", "account_tags", "contact_events", "users")
            for t in tables:
                try:
                    D.execute(f"DELETE FROM {t} WHERE account_id=?", (acct_id,))
                except Exception:  # noqa: BLE001 — table may not exist in older DBs
                    pass
            D.execute("DELETE FROM accounts WHERE id=?", (acct_id,))
        ...
        if request.method == "POST":
            ...
            # ---- Deletion (soft by default, hard as a guarded second step) -- #
            elif action == "delete":
                # Soft delete: hide + block login, fully reversible.
                tgt = get_target(request.form.get("id"))
                if not can_manage(tgt):
                    flash("You're not allowed to delete that user.", "error")
                else:
                    D.execute("UPDATE users SET status='deleted', session_token=NULL"
                              " WHERE id=?", (tgt["id"],))
                    D.log_activity(me["account_id"], me["email"],
                                   f"Soft-deleted user {tgt['email']}")
                    flash("User deleted (recoverable — use Restore).", "success")
            elif action == "restore":
                tgt = get_target(request.form.get("id"))
                if not can_manage(tgt):
                    flash("You're not allowed to manage that user.", "error")
                ...
                # Permanent wipe of the user AND their whole workspace. Extra
                # guarded: super-admin only, never yourself, must confirm the
                # exact email on the front end.
                tgt = get_target(request.form.get("id"))
                confirm = (request.form.get("confirm_email") or "").strip().lower()
                if not is_superadmin():
                    flash("Only a Super Admin can permanently delete a user.", "error")
                elif not can_manage(tgt):
                    flash("You're not allowed to delete that user.", "error")
                elif tgt["id"] == me["id"]:
                    flash("You can't delete your own account.", "error")
                elif confirm != (tgt["email"] or "").lower():
                    flash("Confirmation email did not match — nothing deleted.", "error")
                else:
                    _purge_account(tgt["account_id"])
                    D.log_activity(me["account_id"], me["email"],
                                   f"PERMANENTLY deleted user {tgt['email']} + workspace")
                    flash(f"Permanently deleted {tgt['email']} and all their data.",
                          "success")
            elif action == "bulk_delete":
                # Soft-delete many at once; silently skip protected rows.
                ids = request.form.getlist("ids")
                done = skipped = 0
                for uid in ids:
                    tgt = get_target(uid)
                    if can_manage(tgt) and tgt["id"] != me["id"]:
                        D.execute("UPDATE users SET status='deleted', session_token=NULL"
                                  " WHERE id=?", (tgt["id"],))
                        done += 1
                    else:
                        skipped += 1
                D.log_activity(me["account_id"], me["email"],
                               f"Bulk soft-deleted {done} users")
                flash(f"{done} user(s) deleted." +
                      (f" {skipped} skipped (protected)." if skipped else ""),
                      "success")
