...
def render_html(body, contact, base_url, token):
    ...
    # Per-recipient field values for the Insert-Variable tokens.
    values = {
        "name": name,
    ...
    def _sub(m):
        ...
        return values.get(key) or fallback  # fallback when the field is blank

    # {{field}} or {{field | fallback text}} — anything we know, with a default.
