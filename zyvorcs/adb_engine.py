...
def run_bulk_send(contacts, template, delay, device, status, lock, report_dir,
    ...
    def _within_hours(h):
        """True if hour h is inside the allowed sending window.
        Supports overnight windows (e.g. start=21, end=6)."""
    ...
    try:
        for i, contact in enumerate(contacts):
            ...
            # Sending-hours window: auto-wait until the window opens
            if hours_enforce and not _within_hours(datetime.now().hour):
                _log(status, lock,
                     f'🕘 Outside sending hours ({hours_start:02d}:00–{hours_end:02d}:00) '
                     f'— waiting until window opens...')
                stopped_while_waiting = False
                while not _within_hours(datetime.now().hour):
                    with lock:
                        if status['stop_flag']:
                ...
                if stopped_while_waiting:
                    _log(status, lock, '🛑 Stopped by user')
                    break
                _log(status, lock, '🕘 Sending hours window open — resuming')
            ...
            if i < len(contacts) - 1:
                ...
                # Outside-window warning (only when enforcement is off)
                hour = datetime.now().hour
                if not hours_enforce and not _within_hours(hour):
                    _log(status, lock,
                         f'🌙 Warning: sending outside allowed hours '
                         f'({hours_start:02d}:00–{hours_end:02d}:00) — carrier may flag')
