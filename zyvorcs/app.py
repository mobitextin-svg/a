...
# ─── Custom Buttons ──────────────────────────────────────────────
# Reusable quick-insert buttons for the message box.
# Each button = {id, label, text}. Stored as an ordered list so the
# user can add / update / delete and keep their arrangement.
...
def buttons_handler():
    if request.method == 'POST':
        ...
        btn_id = (data.get('id') or '').strip()

        if not label:
            return jsonify({'success': False, 'message': 'Button label is required'})
        if not text:
            return jsonify({'success': False, 'message': 'Button text is required'})
        ...
        if btn_id:
            ...
            # Add a new button with a unique id
            btn_id = _next_button_id(buttons)
            buttons.append({'id': btn_id, 'label': label, 'text': text})

        _save_buttons(buttons)
        return jsonify({'success': True, 'id': btn_id, 'buttons': buttons})

    return jsonify({'buttons': _load_buttons()})


@app.route('/buttons/<btn_id>', methods=['DELETE'])
def delete_button(btn_id):
    buttons = _load_buttons()
    new_buttons = [b for b in buttons if b.get('id') != btn_id]
    if len(new_buttons) == len(buttons):
        return jsonify({'success': False, 'message': 'Button not found'})
    _save_buttons(new_buttons)
    return jsonify({'success': True, 'buttons': new_buttons})
...
def start_send():
    ...
    sim_mode    = data.get('sim_mode', 'auto')
    ...
    min_delay   = int(data.get('min_delay', 45))
    max_delay   = int(data.get('max_delay', 90))
    if min_delay > max_delay:          # swap if user entered them backwards
        min_delay, max_delay = max_delay, min_delay
    batch_size  = max(0, int(data.get('batch_size', 0)))
    batch_pause = max(5, int(data.get('batch_pause', 30)))
    ...
    if os.path.exists(BUTTONS_FILE):
        try:
            with open(BUTTONS_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
            # Tolerate older / hand-edited files that are not a list
...
def _next_button_id(buttons):
    """Return a unique id like 'b1', 'b2', ... not already used."""
    existing = {b.get('id') for b in buttons}
    i = 1
