...
def create_order(amount_inr, receipt):
    ...
    req = urllib.request.Request(
        "https://api.razorpay.com/v1/orders", data=data,
...
def upi_link(amount, note="MailSaaS burst pack"):
    ...
    name = os.environ.get("UPI_NAME", "MailSaaS")
    return ("upi://pay?pa=" + quote(vpa) + "&pn=" + quote(name)
            + "&am=" + str(amount) + "&cu=INR&tn=" + quote(note))


def bank_details():
    return {"bank": os.environ.get("BANK_NAME", "HDFC Bank (sample)"),
